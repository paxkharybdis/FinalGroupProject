...
{
    public class CustomButton : AbstractGadgetOptions
    {
        ...
        public CustomButton(IGadget gadget, int Qty, GadgetType gadgettype)
            : base(gadget)
        {
            _qty = Qty;
        ...
        {
            get { return customGadget.Price + (_qty * 10.00m); }
        }

        // The following satisfies abstract rules, no functionality otherwise.
        public override int Buttons
        {
            set
            {
                this._buttons = (customGadget.Buttons + _qty);
            }
            get
            {
                return customGadget.Buttons + _qty;
            }
        }
